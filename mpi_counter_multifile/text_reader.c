...
const char *stopwords[] = {
    ...
    "s", "t", "d", "m", "ll", "re", "ve", "com", "http", "https", "www", "html", "gif"
};
...
int is_noise(const char *word) {
    // Detectar URLs o patrones markdown como ![gif](...)
    if (strstr(word, "http") || strstr(word, "www") || strstr(word, ".com") || strstr(word, ".net"))
        return 1;
    ...
    while ((c = fgetc(file)) != EOF) {
        if (isalpha(c)) {
            ...
            if (char_index > 0) {
                ...
                if (!is_stopword(current_word) && !is_noise(current_word)) {
